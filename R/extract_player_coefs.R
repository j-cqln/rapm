extract.player.coefs <- function(data,
  ...
  coeff_rapm <- data.frame(player_name, mins, off_def) %>%
  ...
  if(!"ols" %in% colnames(coeff_rapm)) {
    coeff_rapm$ols = NA
  }
  
  if(!"wols" %in% colnames(coeff_rapm)) {
    coeff_rapm$wols = NA
  }
  
  if(!"ridge" %in% colnames(coeff_rapm)) {
    coeff_rapm$ridge = NA
  }
  
  if(!"lasso" %in% colnames(coeff_rapm)) {
    coeff_rapm$lasso = NA
  }
  
  if(!"pois" %in% colnames(coeff_rapm)) {
    coeff_rapm$pois = NA
  }
  
  coeff_rapm <- coeff_rapm %>%
  ...
  if (collapsed && which_collapsed == "off") {
    coeff_rapm = coeff_rapm %>%
      select(player_name, mins, ols_def, wols_def, ridge_def, lasso_def, pois_def)
  } else if (collapsed && which_collapsed == "def") {
    coeff_rapm = coeff_rapm %>%
